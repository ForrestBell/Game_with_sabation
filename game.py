...
class MyApp:
    import random
    monsterdamage = random.randint(1,35)
    monsterhealth = random.randint(100,200)
    playerhealth = 100
    armor = 50
    healthpack = 20
    punchdmg = 5
    ...
    def __init__(self, parent):
                   	    ...
                   	    self.punch = Button(self.myContainer1)
                   	    self.punch.configure(text="punch", background= "tan")
                   	    self.punch.grid(row=0,column=0)
                   	    # Bind an event to the first button
                   	    self.punch.bind("<Button-1>", self.punchClicked)
                   	    
                   	    self.sword = Button(self.myContainer1)
                   	    self.sword.configure(text="Sword", background= "#E6E7E8")
                   	    self.sword.grid(row=0,column=2)
                   	    # Bind an event to the first button
                   	    self.sword.bind("<Button-1>", self.swordClicked)
                   	    
                   	    self.fireball = Button(self.myContainer1)
                   	    self.fireball.configure(text="Fireball", background= "#9C2A00")
                   	    self.fireball.grid(row=0,column=3)
                   	    # Bind an event to the first button
                   	    self.fireball.bind("<Button-1>", self.fireballClicked)
                   	    
                   	    self.heal = Button(self.myContainer1)
                   	    self.heal.configure(text="heal", background= "#ffd700")
                   	    self.heal.grid(row=0,column=4)
                   	    # Bind an event to the first button
                   	    self.heal.bind("<Button-1>", self.healClicked)          
                            drawpad.pack()
